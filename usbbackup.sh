...
BACKUPS="galaxy_ext_sd:3161-3632 galaxy_int_sd:E5F3-0F04"
...
if [ $BG = 1 ]; then
	...
	for bak in $BACKUPS; do
		...
		logger "Starting backup of disk $uuid ($DEVNAME) to $TARGET/$name"
		if [ ! -d /tmp/$uuid ]; then
		    mkdir /tmp/$uuid
		fi
		...
		umount /tmp/$uuid
		logger "Finished backup of disk $uuid ($DEVNAME) to $TARGET/$name"
	    fi
	done
    fi
else
